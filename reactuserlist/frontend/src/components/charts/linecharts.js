...
const date1 =
 ...
 const Linecharts = () => {
    ...
    var totline = date1.map(
        tl => (
            tl.data
        )
    )

    console.log("totline",totline)
    var name = date1.map(
        n => (
            n.name
        )
    ...
    var chartsdate = date1.map(a => (
        a.data.map((b => (
       console.log('')//  console.log("month",new Date(b[0]).toUTCString())
        )))
    ))
    var chartsvalue = date1.map(a => (
        a.data.map((b => (
            b[1]
        )))
    ))
    console.log("chartsdate",chartsdate);
        ...
        <div>
            ...
            <LineChart data={date1}>
               ...
               {/* {
                ...
                <YAxis stroke="blue"/>
                <Line  dataKey="name" data={chartsvalue} stroke="black" activeDot={{ r: 8 }}/>
                <Legend />
                <Tooltip />
            </LineChart>
        </ResponsiveContainer>
